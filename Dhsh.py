...
def process_multi_bin(message):
    ...
    # تشفير النمط
    encoded_pattern = encode_pattern(pattern)
    gen_states[user_id] = {'pattern': pattern, 'type': 'pattern'}
    keyboard = get_count_keyboard(encoded_pattern, 'pattern')
...
⭐ Total Points: {total_points}"""
    ...
    if data.startswith("gen_count_"):
    	parts = data.split('_')
    	gen_type = parts[2]
    	param = '_'.join(parts[3:-1])  # جمع كل الأجزاء ما عدا آخر جزء (العدد)
    	count = int(parts[-1])
    
    # إذا كان النوع pattern، فك تشفير param
    if gen_type == 'pattern':
        try:
            param = decode_pattern(param)
        except:
            pass
        ...
        if gen_type == 'single':
            ...
            # إذا كان العدد أكثر من 10، أرسل الملف مباشرة
            if count > 10:
