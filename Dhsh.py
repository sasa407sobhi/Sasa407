...
def get_check_menu():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("💳 Stripe 5", callback_data="check_str5"),
        InlineKeyboardButton("💳 Stripe 6", callback_data="check_str6")
    )
    keyboard.add(
...
def get_gen_menu():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("🎭 Custom Pattern", callback_data="gen_pattern"),
...
def get_mass_gateways_menu():
    ...
    gateways = [
        ...
        ("PayPal 0.01$", "mass_pp1"), ("PayPal 1$", "mass_pp2")
    ]
...
def format_result(gateway_name, card, status, response, exec_time):
    ...
    bin_info_clean = re.sub(r'<[^>]+>', '', bin_info)
    return f"""
┏━━━━━━━━━━━━━━━━━━━━┓
┃ #{gateway_name}
┣━━━━━━━━━━━━━━━━━━━━┫
...
def generate_cards_count(bin_input, count):
...
def cmd_str5(m): 
    user_id = m.from_user.id
    card = m.text.replace('/str5', '').strip()
    if not card:
        bot.reply_to(m, "⚠️ /str5 number|month|year|cvv")
        return
    check_single_card(user_id, card, Stripe5, "Stripe 5")

@bot.message_handler(commands=['str6'])
def cmd_str6(m): 
    user_id = m.from_user.id
    card = m.text.replace('/str6', '').strip()
    if not card:
        bot.reply_to(m, "⚠️ /str6 number|month|year|cvv")
        return
    check_single_card(user_id, card, Stripe6, "Stripe 6")
...
def process_pattern(message):
    ...
    if '|' not in pattern:
        bot.reply_to(message, "❌ Invalid pattern\nExample: 410621xxxx|12|29|xxx")
        return
    
    gen_states[user_id] = {'pattern': pattern, 'type': 'pattern'}
    keyboard = get_count_keyboard(pattern.replace('|', '_'), 'pattern')
    bot.reply_to(message, "🎭 Choose number of cards:", reply_markup=keyboard)
...
📌 Each check costs 1 point"""
    ...
    if data == "gen_pattern":
        bot.edit_message_text("🎭 Send pattern\nExample: 410621xxxx|12|29|xxx", user_id, call.message.message_id)
...
⭐ Total Points: {total_points}"""
    ...
    if data.startswith("gen_count_"):
        ...
        if gen_type == 'single':
            cards = generate_cards_count(param, count)
            ...
            keyboard.add(
                ...
                InlineKeyboardButton("📁 Save as TXT", callback_data="save_gen_cards")
            )
            keyboard.add(InlineKeyboardButton("🔙 Back", callback_data="gen_menu"))
            ...
            all_cards = []
            for b in bins[:5]:
                cards = generate_cards_count(b, count)
                all_cards.extend(cards)
            cards_text = '\n'.join([f"<code>{c}</code>" for c in all_cards[:100]])
            ...
            pattern = param.replace('_', '|')
            pattern_parts = pattern.split('|')
            bin_pattern = pattern_parts[0].strip()
            month = pattern_parts[1].strip().lower()
            year = pattern_parts[2].strip().lower()
            cvv_pattern = pattern_parts[3].strip().lower()
            ...
            for _ in range(min(count, 200)):
                ...
                m = str(random.randint(1, 12)).zfill(2) if month == 'xx' else month.zfill(2)
                y = str(random.randint(2026, 2031)) if year == 'xx' else (f"20{year}" if len(year) == 2 else year)
                cvv = ''.join(random.choices('0123456789', k=3)) if cvv_pattern.lower() == 'xxx' else cvv_pattern
                cards.append(f"{card_num}|{m}|{y}|{cvv}")
            
            cards_text = '\n'.join([f"<code>{c}</code>" for c in cards])
            gen_states[user_id] = {'cards': cards, 'type': 'pattern'}
    ...
    if data == "copy_gen_cards":
        if user_id in gen_states:
            cards = gen_states[user_id]['cards']
            cards_text = '\n'.join(cards)
    ...
    if data.startswith("fake_"):
        ...
        info = generate_fake_info(country)
        bot.edit_message_text(info, user_id, call.message.message_id, parse_mode='HTML', reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("🔙 Back", callback_data="fake_menu")))
        bot.answer_callback_query(call.id)
        return
    ...
    single_checks = {
        "check_str1": Stripe1, "check_str2": Stripe2, "check_str3": Stripe3,
        "check_str4": Stripe4, "check_str5": Stripe5, "check_str6": Stripe6,
        "check_chb": BraC,
        "check_live1": charge1usd,
        "check_pp1": check_paypal_gate1, "check_pp2": check_paypal_gate2
    }
    ...
    # ========== الفحص الجماعي ==========
    mass_gateways = {
        "mass_str1": Stripe1, "mass_str2": Stripe2, "mass_str3": Stripe3,
        "mass_str4": Stripe4, "mass_str5": Stripe5, "mass_str6": Stripe6,
        "mass_chb": BraC,
        "mass_live1": charge1usd,
        "mass_pp1": check_paypal_gate1, "mass_pp2": check_paypal_gate2
    }
